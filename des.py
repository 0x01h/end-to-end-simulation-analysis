...
def order_cabbage():
    order_time = random.custom_uniform(1, 15)  # Order arrival time for coming cabbages
    return order_time


def order_arrived(available_cabbages):
    available_cabbages += 1
    rotten_time = random.custom_uniform(7, 12)  # Rotten time for arrived cabbages
    return available_cabbages, rotten_time


def cus_entry(available_cabbages, happy_cus_left, sad_cus_left):
    cus_arr_time = random.custom_uniform(0, 3)  # Customer arrival time after one customer left
...
def simulation(stock, terminators, verbose_fel=False, test_mode=False):
    ...
    for _ in range(0, stock):
        init_rotten_time = random.custom_uniform(7, 12)
